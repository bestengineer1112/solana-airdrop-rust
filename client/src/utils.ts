...
import {
  ...
  ExpectedTokenAirdropAccountLayoutInterface,
} from "./account";
...
const envItems = [
  ...
  "TOKEN_AIRDROP_PROGRAM_ACCOUNT_PUBKEY",
];
...
export const checkAccountInitialized = async (
  ...
  if (customAccount === null || customAccount.data.length === 0) {
    console.error(
      "Account of custom program has not been initialized properly"
    );
    process.exit(1);
  }
...
export const checkAccountDataIsValid = (
  customAccountData: TokenAirdropAccountLayoutInterface,
  expectedCustomAccountState: ExpectedTokenAirdropAccountLayoutInterface
) => {
